...
## Design Coverage Index (P1.5 items, no omissions)

This plan closes the remaining “hard invariants” from:
- `docs/plans/2026-01-25-opencode-sandbox-context-design.md`

Covered design sections (P1.5 scope):
- Section 8.1: emit `evidence.write_failed` (no silent evidence loss)
- Section 15: enrich `pack.json` provenance + ensure view (`pack.md`) points to key artifacts
- Section 2.2: isolated workdir produces a first-class change-set artifact (patch/diff)
- Section 13.1.1: exec-policy evaluation as an auditable artifact (`execpolicy.eval.json`)
- Section 12 + Section 6: safe `opencode evidence export` (allowlist + no symlinks) + export events
- Decision Log / invariants: “max 1 approval prompt per tool run” (approval consolidation)

Explicitly not covered (track as P2.x if still missing after P2):
- Section 10: `UPSTREAM.lock.json` is generated by `opencode-zh-build/update_opencode_zh.sh` (outside this repo plan). Ensure the lockfile exists and is referenced by evidence/provenance or CI checks.
