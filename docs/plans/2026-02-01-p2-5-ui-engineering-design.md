...
> 这一节是把评审意见转成“可实现的产品约束”，用来避免做偏或返工。

#### 2.1.1 高频事件导致的“频闪/抽搐”（信息过载）

**风险：** 当系统在短时间内产出多条事件（尤其是工具 start/end、policy/sandbox 噪音、子任务并行），如果 UI 每次都立即更新胶囊文案，会造成“频闪”——用户感知为焦虑与失控，而不是掌控感。

**对策：**
- Live Capsule 的“主状态文案”必须做 **视觉节流（Visual Throttling）**：
  - 同类状态更新至少保持 **800ms–1200ms** 不变；
  - 只有在“重大状态跳变”时允许立刻更新（如：`failed/needs_attention`、从 `running → done`、或出现需要用户介入的事件）。
- Timeline 展开时也必须控制密度：对“动作数量过多”的 turn，默认只展示前 5 + 后 2，中间折叠为“还有 N 个动作”（可展开）。

#### 2.1.2 Capsule 的位置与心理模型必须正确

**原则：** Capsule 属于 “用户说完 → 系统执行 → 系统回答” 中间的 **Void Between Turns**，它应该 **紧贴用户消息之后**，而不是挂在 assistant 输出上（避免空输出/边界条件导致位置跳动或消失）。

**对策：**
- App 侧在 `SessionTurn` 内提供一个“addon slot”，把 Capsule 固定渲染在 **用户消息之后** 的稳定区域。

#### 2.1.3 失败/阻塞不能被折叠掩盖

**风险：** 如果失败也自动折叠，用户会看到“已完成”但没有可用结果，体验断层。

**对策：**
- 若 turn 内存在 `failed` 或 `needs_attention` 动作，默认 **自动展开** timeline（直到用户手动折叠）。
- 失败态必须在视觉上强可见（色彩/标签/置顶），并提供下一步（复制指针、打开审计、跳到相关消息）。

#### 2.1.4 审计视图的性能与懒加载

**风险：** 将 raw JSON（可能很大）直接挂载到 DOM（哪怕折叠）会造成内存与卡顿风险。

**对策：**
- “审计（Inspect）”内容必须 **懒渲染**：只有在用户打开 Drawer/Popover 时才渲染 raw JSON。
- 默认视图只渲染 title/summary/duration/pointers（安全小字段）。

#### 2.1.5 子任务不应只在 Header 里“孤岛式存在”

**风险：** 如果子任务只在 Header 有一个入口，主叙事（turn capsule）看不见并行任务，会造成“割裂”。

**对策：**
- Live Capsule 的摘要中必须能反映：`并行子任务 ×N`（至少 session 级别）。
- 子任务完成提醒在主聊天流中必须是“可点击、可导航、中文可读”的卡片，而不是工程文本。

#### 2.1.6 动态叙事标题（低成本高感知）

**目标：** 让用户感觉“AI 知道自己在做什么”，而不是“日志翻译器”。

**对策：**
- 基于事件类型/actor 的轻量映射（不读取敏感原文），输出中文动词句：
  - `tool:bash` → “正在执行命令… / 已执行命令”
  - `doc.extract_pdf_text` → “正在提取 PDF 文本…”
  - `worktree.merge_*` → “正在合并变更…”
  - `gate.*` → “正在进行质量检查…”
- 更细粒度（如解析命令语义：npm install/git commit/rg）可作为后续抛光，但 v1 先做到“去工程腔 + 可理解”。

### 2.2 世界级升华点（Magic Moments）

> 这一节是把两份 Gemini 二次评审的“升华点”合并为一套可落地的产品体验增强。  
> 原则：**以事实事件为锚**（evidence/parts/状态），再做叙事与动效；避免“UI 自说自话”产生幻觉。

#### 2.2.1 有机思考 vs 机械执行（Organic vs Mechanical）

用户的核心心理预期是：“它在想”还是“它在做”。两者必须在 UI 上一眼可区分：
- **思考/规划（Routing/Planning）**：呼吸/脉冲（柔和、圆润、偏紫/蓝），文案为“正在拆解任务/正在规划方案…”
- **执行/操作（Tools/Workbench/Sandbox）**：确定性的机械反馈（更锐利、对比更强、偏冷/执行色），文案为“正在执行命令/正在解析附件…”

落地建议：
- Live Capsule 的 icon/动效随 `phase` 切换（routing → pulse；tool/doc → spin/flicker；stalled → arrhythmia）。
- 同时把“动作名”人话化，避免暴露 `tool:bash`、`routing.*` 这类工程词。

#### 2.2.2 产物即英雄（Artifact Heroizing）

完成摘要不应该只有“工具×4”，而应该优先体现用户关心的产物：
- 若本 turn 产生了文件变更：摘要里直接展示 **1–2 个文件图标 + 文件名**（其余折叠为“+N”）。
- 若产生了关键证据/快照：展示一个轻量 badge（如“证据已记录”），并提供复制指针入口。

落地建议：
- 使用 `message.summary.diffs`（或 session_diff）提取“本 turn 的产物”，渲染为 mini chips。
- 仍保留审计视图里的完整列表与指针。

#### 2.2.3 焦虑管理 / 预期管理（Anxiety Management）

长耗时本身不是问题，“不知道发生什么”才会让用户焦虑。

落地建议：
- 0–2s：正常状态文案（例如“正在执行…”）
- 2–5s：增加预期提示（例如“正在运行测试套件（可能需要一些时间）…”）
- >10s：明确“仍在运行中，请稍候…”，并显示计时器（不是绝对时间戳）
- >20s 且无新事件：进入“卡顿/重试”语义（不必说具体原因，避免幻觉）

#### 2.2.4 子任务的空间在场感（Spatial Presence）

子任务不应只在 Header 里“孤岛式存在”。主叙事里必须能“看见它还在后台干活”：
- Live Capsule 在 running 时，可展示一个 **极小的 Subtask Pip**（最多 2 个，更多显示“+N”）。
- Pip 完成时自然过渡为“完成卡片”（同一视觉语言），并支持一键跳转到子会话。

#### 2.2.5 完成的确认感（Completion Linger）

不要让 Capsule 完成后立刻塌缩消失：用户需要一个“我看见你完成了”的确认瞬间。

落地建议：
- 完成后先显示 `✅ 已完成` 并停留 ~1.5s，再优雅收缩为单行摘要（可点击展开）。

#### 2.2.6 冰山时间线（Iceberg Timeline）

Timeline 的默认阅读应像“冰山”：先看一角，再按需深入。

落地建议：
- 默认只展示每个动作的 header（图标 + 人话标题 + 耗时 + 状态点）
- 点击 header 才 inline 展开详情（Accordion）；不要默认弹出大 Dialog（Dialog 留给审计）
- 当动作数量过多：默认展示前 5 + 后 2，中间折叠为“还有 N 个动作”
- 时间戳：产品视图只展示“耗时 / 相对起点时间（+1.2s）”，绝对时间只放审计视图

### 2.3 系统提示词的最小增强（可选，不作为 P2.5 依赖）

P2.5 的默认体验必须在“不改系统提示词”的前提下成立（因为 UI 以证据事件为真相来源）。

但如果允许做极小增强，可考虑新增一个受控、可忽略的 UI Hint 通道：
- 允许模型在工具调用前输出极短 `<ui_intent>`（<= 5–8 个中文词）
- UI 若解析到该 tag，可作为 capsule 的第一帧文案（填补 routing/latency 空窗）
- 若模型不输出则回退到事件映射（不影响正确性）
